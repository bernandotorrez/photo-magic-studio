-- =====================================================
-- ADD COMPLETE ENHANCEMENTS TO DATABASE
-- Migration to add all enhancements from API_ENHANCEMENT_LIST.md
-- =====================================================

-- =====================================================
-- PART 1: ADD FOOD ENHANCEMENTS
-- =====================================================
...
-- =====================================================
-- PART 2: ADD FASHION ENHANCEMENTS
-- =====================================================
...
-- =====================================================
-- PART 3: ADD INTERIOR ENHANCEMENTS
-- =====================================================
...
-- =====================================================
-- PART 4: ADD EXTERIOR ENHANCEMENTS
-- =====================================================
...
-- =====================================================
-- PART 5: ADD PORTRAIT ENHANCEMENTS
-- =====================================================
...
-- =====================================================
-- PART 6: UPDATE CATEGORY MAPPINGS
-- =====================================================

-- Add food category if not exists
...
-- Add portrait category if not exists
INSERT INTO public.image_categories (category_code, category_name, description, icon, sort_order) VALUES
('portrait', 'Portrait & People', 'Portrait photography, people, and human subjects', '👤', 8)
ON CONFLICT (category_code) DO NOTHING;

-- Map food enhancements to food category
DO $$
DECLARE
  food_cat_id UUID;
  enh_id UUID;
BEGIN
  SELECT id INTO food_cat_id FROM public.image_categories WHERE category_code = 'food';
  
  IF food_cat_id IS NOT NULL THEN
    FOR enh_id IN 
      SELECT id FROM public.enhancement_prompts WHERE category = 'food' AND is_active = true
    LOOP
      INSERT INTO public.category_enhancements (category_id, enhancement_id, sort_order)
      SELECT food_cat_id, enh_id, sort_order
      FROM public.enhancement_prompts WHERE id = enh_id
      ON CONFLICT (category_id, enhancement_id) DO UPDATE SET
        sort_order = EXCLUDED.sort_order;
    END LOOP;
  END IF;
END $$;

-- Map fashion enhancements to clothing category
DO $$
DECLARE
  clothing_cat_id UUID;
  enh_id UUID;
BEGIN
  SELECT id INTO clothing_cat_id FROM public.image_categories WHERE category_code = 'clothing';
  ...
  IF clothing_cat_id IS NOT NULL THEN
    ...
    LOOP
      INSERT INTO public.category_enhancements (category_id, enhancement_id, sort_order)
      SELECT clothing_cat_id, enh_id, sort_order
      FROM public.enhancement_prompts WHERE id = enh_id
      ON CONFLICT (category_id, enhancement_id) DO UPDATE SET
        sort_order = EXCLUDED.sort_order;
    END LOOP;
...
-- Map interior enhancements to interior category
DO $$
DECLARE
  interior_cat_id UUID;
  enh_id UUID;
BEGIN
  SELECT id INTO interior_cat_id FROM public.image_categories WHERE category_code = 'interior';
  
  IF interior_cat_id IS NOT NULL THEN
    FOR enh_id IN 
      SELECT id FROM public.enhancement_prompts WHERE category = 'interior' AND is_active = true
    LOOP
      INSERT INTO public.category_enhancements (category_id, enhancement_id, sort_order)
      SELECT interior_cat_id, enh_id, sort_order
      FROM public.enhancement_prompts WHERE id = enh_id
      ON CONFLICT (category_id, enhancement_id) DO UPDATE SET
        sort_order = EXCLUDED.sort_order;
    END LOOP;
...
-- Map exterior enhancements to exterior category
DO $$
DECLARE
  exterior_cat_id UUID;
  enh_id UUID;
BEGIN
  SELECT id INTO exterior_cat_id FROM public.image_categories WHERE category_code = 'exterior';
  
  IF exterior_cat_id IS NOT NULL THEN
    FOR enh_id IN 
      SELECT id FROM public.enhancement_prompts WHERE category = 'exterior' AND is_active = true
    LOOP
      INSERT INTO public.category_enhancements (category_id, enhancement_id, sort_order)
      SELECT exterior_cat_id, enh_id, sort_order
      FROM public.enhancement_prompts WHERE id = enh_id
      ON CONFLICT (category_id, enhancement_id) DO UPDATE SET
        sort_order = EXCLUDED.sort_order;
    END LOOP;
...
-- Map portrait enhancements to portrait category
DO $$
DECLARE
  portrait_cat_id UUID;
  enh_id UUID;
BEGIN
  SELECT id INTO portrait_cat_id FROM public.image_categories WHERE category_code = 'portrait';
  
  IF portrait_cat_id IS NOT NULL THEN
    FOR enh_id IN 
      SELECT id FROM public.enhancement_prompts WHERE category = 'portrait' AND is_active = true
    LOOP
      INSERT INTO public.category_enhancements (category_id, enhancement_id, sort_order)
      SELECT portrait_cat_id, enh_id, sort_order
      FROM public.enhancement_prompts WHERE id = enh_id
      ON CONFLICT (category_id, enhancement_id) DO UPDATE SET
        sort_order = EXCLUDED.sort_order;
    END LOOP;
...
-- Check total enhancements added
SELECT 
  category,
